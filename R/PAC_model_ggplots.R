...
#' 
#' @example
# water <- suppressWarnings(define_water(doc=2.5, uv254=.05,toc=3.5)) %>%
...
results <- data.frame(dose=numeric(), time=numeric(), predicted_doc=numeric())


#loop through data frame to calculate predicted_doc 
for (d in doses){
  for (t in times) {
    predicted_doc <- pac_toc(water, d=doses, t=times, type="bituminous")@doc
    
    results <-rbind(results,data.frame(d=doses,t=times, predicted_doc=predicted_doc))
  }
}
...
ggplot(results, aes(x=doses, y=predicted_doc, color=as.factor(times)))+
  geom_line() +
  labs(
