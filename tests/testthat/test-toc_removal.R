...
test_that("chemdose_toc returns the same water when coagulant dose is 0.", {
  ...
  expect_equal(water1, suppressWarnings(chemdose_toc(water1)))
  expect_equal(water2, suppressWarnings(chemdose_toc(water2)))
})
...
test_that("chemdose_toc works.", {
  ...
  water3 <- suppressWarnings(chemdose_toc(water1, fecl3 = 50, coeff = "Ferric"))
  water4 <- suppressWarnings(chemdose_toc(water1, fecl3 = 50,
  ...
  # data = data.frame(DOC = 3.5, dose = convert_units(50, "fecl3", endunit = "mM"), pH = 7, UV254 = .1)
