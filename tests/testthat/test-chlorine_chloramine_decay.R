...
test_that("chemdose_cl2 returns modeled chlorine/chloramine residual = 0 when chlorine dose is 0.", {
  ...
  Ct <- suppressWarnings(chemdose_cl2(water1, cl2_dose = 0, time = 8))
...
test_that("chemdose_cl2 does not run when treatment_type isn't supplied correctly.", {
  ...
  expect_error(chemdose_cl2(water1, cl2_dose = 1, time = 1, treatment = "rw"))
  expect_error(chemdose_cl2(water1, cl2_dose = 1, time = 1, treatment = treated))
})
...
test_that("chemdose_cl2 warns when inputs are out of model range", {
  ...
  expect_warning(chemdose_cl2(water1, cl2_dose = 0.994, time = 1)) # chlorine out of bounds
  expect_warning(chemdose_cl2(water1, cl2_dose = 2, time = 121, treatment = "coag")) # time out of bounds
  expect_warning(chemdose_cl2(water2, cl2_dose = 0.995, time = 100)) # toc out of bounds
  expect_warning(chemdose_cl2(water3, cl2_dose = 2, time = 100, treatment = "coag")) # uv254 out of bounds
})
