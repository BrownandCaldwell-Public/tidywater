...
test_that("gac_toc_once outputs are the same as base function, gac_toc", {
...
test_that("gac_toc_once output is data frame", {
  testthat::skip_on_cran()
...
test_that("gac_toc_chain outputs are the same as base function, gac_toc", {
  testthat::skip_on_cran()
  water0 <- define_water(7.9, 20, 50,
                         tot_hard = 50, ca = 13, mg = 4,
                         na = 20, k = 20, cl = 30, so4 = 20,
  ...
  water2 <- water_df %>%
    slice(1) %>%
    define_water_chain() %>%
    gac_toc_chain(model = "WTP", bed_vol = 15000, media_size = "12x40", ebct = 10, output_water = "gac") %>%
    pluck_water("gac", c("doc", "toc", "uv254"))
...
test_that("gac_toc_chain output is list of water class objects", {
  testthat::skip_on_cran()
...
# Check gac_toc_chain can use a column or function argument for chemical dose and both methods gives same results
test_that("gac_toc_chain can use a column or function argument for chemical dose", {
  testthat::skip_on_cran()
  water1 <- suppressWarnings(water_df %>%
                               slice(1) %>%
                               define_water_chain("raw") %>%
                               gac_toc_chain(input_water = "raw", model = "WTP", bed_vol = 15000) %>%
                               pluck_water("gac_water", c("doc", "uv254")))
  ...
  water3 <- suppressWarnings(water_df %>%
                               ...
                               define_water_chain("raw") %>%
                               mutate(model = "WTP") %>%
                               gac_toc_chain(input_water = "raw", bed_vol = 15000) %>%
                               pluck_water("gac_water", c("doc", "uv254")))
  
  expect_equal(water1$gac_water_doc, water2$gac_water_doc) # test different ways to input args
  expect_equal(water1$gac_water_uv254, water2$gac_water_uv254)
  
  # Test that inputting time/dose separately (in column and as an argument)  gives same results
  expect_equal(water1$gac_water_doc, water3$gac_water_doc)
})

# Check that errors with argument + column for same param
test_that("gac_toc_chain errors with argument + column for same param", {
  testthat::skip_on_cran()
  water <- water_df %>%
    define_water_chain("raw")
  expect_error(water %>%
...
test_that("gac_toc_chain correctly handles arguments with multiple numbers", {
  testthat::skip_on_cran()
  water <- water_df %>%
    define_water_chain("raw")

  water1 <- water %>%
    gac_toc_chain("raw", model = "Zachman", bed_vol = c(8000, 10000))
  water2 <- water %>%
    gac_toc_chain("raw", model = c("Zachman", "WTP"), bed_vol = 10000)
