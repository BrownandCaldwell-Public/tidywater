...
test_that("Balance ions results in neutral charge.", {
  ...
  water5 <- balance_ions(water3, anion = "mg")
  ...
  water6 <- balance_ions(water1, anion = "h2po4")
...
test_that("balance_ions_once output is the same as balance_ions", {
  ...
  water5 <- balance_ions(water4, anion = "hco3", cation = "ca")

  water6 <- suppressWarnings(define_water_chain(slice(water_df, 1))) %>%
    balance_ions_once(anion = "hco3", cation = "ca") %>%
    select(-defined_water)
...
test_that("balance_ions_chain outputs are the same as base function, balance_ions", {
  ...
  water6 <- balance_ions(water5, anion = "co3", cation = "mg")

  water7 <- suppressWarnings(define_water_chain(slice(water_df, 1))) %>%
    balance_ions_chain(anion = "co3", cation = "mg")
