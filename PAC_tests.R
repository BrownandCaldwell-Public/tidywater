library(testthat)
library(tidywater)
#install use this to set up test file
intall.packages("usethis")
library(usethis)
usethis:use_test("Pac_toc")
test_check("tidywater")
...
test_that("pac_toc returns no modeled DOC value when PAC dose is 0.", {
  ...
  water2 <- suppressWarnings(pac_toc(water1,type="wood", time = 8,dose=0))
  #expected value of doc when no PAC is added - would equal starting value 
  expect_equal(water2@doc, 2.5)
})

#test that function handles wrong pac type - example is "invalid" 
test_that("pac_toc does not run when type isn't specified correctly.", {
  water1 <- suppressWarnings(define_water(doc=2.5, uv254=.05,toc=3.5))
  
  expect_error(pac_toc(water1, dose=15, time=50, type = "invalid"))
})

test_that("pac_toc warns when inputs are out of model range", {
  water1 <- suppressWarnings(define_water(doc=2.5, uv254=.05,toc=50))
  
  expect_warning(pac_toc(water1, dose=31, time = 50)) # dose is out of bounds
  expect_warning(pac_toc(water1, dose=15, time = 1441)) # time out of bounds
...
water1 <-supressWarnings(define_water(doc=3.5,uv254=.1))
dosed_water <- pac_toc(water1, dose=15)
...
test_that("pac_toc works", {
  ...
  water2 <- pac_toc(water1, dose=15, time = 50)
  expect_equal(round(water2@doc, 2), .86)
  expect_equal(round(water2@uv254,3), .86) 
  
  water3 <- pac_toc(water1, dose=15, time=50, type="wood")
  expect_equal(round(water3@doc,2),.86)
  
  water4 <- pac_toc(water1, dose=15, time=50, type="lignite")
  expect_equal(round(water4@doc,2), .86)
})

#check expect equal values above, these are just placeholders for now
