...
The influent water object now has populated slots corresponding to predicted DBP concentrations, including tthm for total trihalomethanes and haa5 for sum of haloacetic acids. However, when comparing these results to the facility's record, we realize that the default model coefficients used in `chemdose_dbp` don't accurately predict DBP formation at this facility. We can still use `chemdose_dbp`; we just need to fit coefficients customized for this facility.

## Fitting DBP coefficients

We can use the `optim()` function to fit custom coefficients to the facility's DBP data. In this example, let's say that only the predictions for chloroform are not accurate. First, let's visualize the facility data against the default DBP predictions.

```{r initial-plot, echo=TRUE, message=FALSE, warning=FALSE}
# Given sample data
dbp_data <- data.frame(ph = c(7, 7.04, 7.5, 7.9, 7.2, 7.25),
                       temp = rep(20, 6),
...
data_again <- data.frame(ph = c(7, 7.04, 7.5, 7.9, 7.2, 7.25),
                       temp = rep(20, 6),
                       alk = c(50, 66, 75, 80, 55, 100),
                       toc = c(4, 8, 4, 10, 5, 6),
                       uv254 = c(0.1, 0.2, 0.2, 0.1, 0.05, 0.2),
                       br = rep(50, 6),
                       fin_chcl3 = c(65, 136.5, 70, 205, 84.5, 103)) %>%
  define_water_df("input")

test <- optim(par = c(6.237e-2, 1.617, -0.094, -0.175, 0.607, 1.403, 0.306), fn = model_fn, data = data_again, control = list(maxit = 100))
coeffs <- data.frame(ID = "chcl3",
                     A = test$par[1],
                     a = test$par[2],
                     b = test$par[3],
                     c = test$par[4],
...
dbp_data <- dbp_data %>%
  chemdose_dbp_df(input_water = "input", output_water = "coeff_disinfected", coeff = coeffs) %>%
  pluck_water(input_waters = c("coeff_disinfected"), parameter = c("chcl3"))

ggplot(data = dbp_data)+
  geom_point(aes(x = fin_chcl3, y = disinfected_chcl3, color = "Default coeffs")) +
  geom_point(aes(x = fin_chcl3, y = coeff_disinfected_chcl3, color = "Custom coeffs")) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "red") +
  labs(title = "Observed vs predicted chloroform",
       x = "Observed chloroform",
