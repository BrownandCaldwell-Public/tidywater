...
raw_wells_water <- tibble(Well = c("A", "B"),
  ph = c(8, 9),
  alk = c(100, 150),
  temp = c(18, 19),
  ca_hard = c(5, 10),
...
blended_wells_water <- blend_waters(waters = c(pluck(raw_wells_water, "balanced_water", 1),
...
scenarios <- tibble(surface_flow = seq(1, 20, 1),
...
River_water <- tibble(ph = 7, temp = 20, alk = 200, tds = 950, cond = 1400,
  tot_hard = 300, na = 100, cl = 150, so4 = 200) %>%
  define_water_chain() %>%
  balance_ions_chain(output_water = "river") %>%
  select(-defined_water)

Lake_water <- tibble(ph = 7.5, temp = 19, alk = 180, tds = 900, cond = 1000,
...
# Plot the results!
ggplot(plotting_data, aes(x = total_flow, y = tot_hard, color = as.character(Wells_flow), shape = Flagged)) +
  geom_point() +
