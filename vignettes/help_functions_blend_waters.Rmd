...
To finish blending our wells, we will transform the `blended_wells` `water` object into a data frame containing a `water` column. First, we'll use the `convert_water` function to to take the input `water` object and convert it to a data frame. Then we'll run `define_water_chain` to make a data frame with water column, named "wells". Most of tidywater's `_chain` functions have the option to name the output column. Defaults vary depending on the `_chain` function. 
